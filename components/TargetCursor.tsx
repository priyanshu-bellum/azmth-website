...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const tickerFn = () => {
      if (!cursor || !activeTarget || !cornersRef.current) return;
      ...
      gsap.set(cursor, { x: targetCenterX, y: targetCenterY });

      const strength = activeStrengthRef.current.current;
      const corners = Array.from(cornersRef.current);
      
      corners.forEach((corner, i) => {
        const targetX = targets[i].x - targetCenterX;
        const targetY = targets[i].y - targetCenterY;
        const followSpeed = strength > 0.9 ? 0.3 : 0.1;
    ...
    const scrollHandler = () => {
      ...
      if (!isStillOverTarget && !isActiveRef.current) {
        currentLeaveHandler?.();
      }
    };
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      const targetColor = window.getComputedStyle(target).color || '#ffffff';
      gsap.to(cursorRef.current, {
        '--cursor-color': targetColor,
        duration: 0.3,
        overwrite: 'auto'
      });
      ...
      const leaveHandler = () => {
        if (!activeTarget || activeTarget.classList.contains('is-imploding')) return;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ willChange: 'transform', borderColor: 'var(--cursor-color)' }}
      />
      <div
